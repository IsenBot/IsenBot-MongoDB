...
module.exports = async (interaction) => {
    ...
    if (limit > 25 || limit < 1) return interaction.reply({ content: await interaction.translate('music/music/error:exe:limit_out_of_range', { min: 0, max: 100 }), ephemeral: true });
    ...
    if (!data?.data?.length) return interaction.reply({ content: await interaction.translate('music/music/error:exe:404_result'), ephemeral: true });
